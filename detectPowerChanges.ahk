...
OnPBMsg(wParam, lParam, msg, hwnd) {
	If (wParam = 0) {	;PBT_APMQUERYSUSPEND
		If (lParam & 1)	;Check action flag
			;The computer is trying to suspend, and user interaction is permitted.
		Else ;The computer is trying to suspend, and no user interaction is allowed.
		;Return TRUE to grant the request, or BROADCAST_QUERY_DENY to deny it.
	} Else If (wParam = 2)	;PBT_APMQUERYSUSPENDFAILED
		; The computer tried to suspend, but failed.
	Else If (wParam = 4)	;PBT_APMSUSPEND
		; The computer is about to enter a suspended state.
	Else If (wParam = 6)	;PBT_APMRESUMECRITICAL
		; The computer is now resuming from a suspended state, which may have taken place unexpectedly.
	Else If (wParam = 7)	;PBT_APMRESUMESUSPEND
		; The computer is now resuming from a suspended state.
	Else If (wParam = 9)	;PBT_APMBATTERYLOW
		; The computer battery is running low.
	Else If (wParam = 10)	;PBT_APMPOWERSTATUSCHANGE
		; The computer power status has changed.
	Else If (wParam = 11)	;PBT_APMOEMEVENT
		; The APM BIOS has signaled an APM OEM event with event code %lParam%
	Else If (wParam = 18)	;PBT_APMRESUMEAUTOMATIC
		; The computer is now automatically resuming from a suspended state.
