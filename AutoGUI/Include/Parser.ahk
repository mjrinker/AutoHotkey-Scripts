    		...
    		If (RegExMatch(A_LoopField, "iO)^\s*?Gui\s*\,?\s*(\w+\:)?\s?Add,\s?(?P<Type>([A-Za-z]+)\d?),\s?(?P<Options>([\w\+\-\s]+))(,\s?(?P<Text>(.+)))?", Rex)) {
    			...
    			If (Rex.Type = "StatusBar" && StatusBarExist()) {
    				...
    				Try {
    					...
    					, % "hWndhWnd " . Position . " " . Options . " " . Style . " " . ExStyle, % Rex.Text
    				} Catch e {
    					MsgBox 0x10, Error
    					, % RexType . ", " Position . " " . Options . " " . Style . " " . ExStyle . " " . Rex.Text
    				}
    			}
    			...
    			Register(hWnd, Rex.Type, "", Rex.Text, cx, cy, cw, ch, hWndVar, vVar, gLabel, Options, Style, ExStyle, "", "", TabPos)
