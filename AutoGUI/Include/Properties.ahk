...
ShowProperties:
    ...
    Gui Add, Tab3
    , hWndhPropTab vCurrentTab x6 y41 w270 h303 AltSubmit %g_ThemeFix%, General|Options|Window|Events|Script
    ...
    Gui Tab, 3 ; Window
        ...
        g_WinColor := ToHex(CvtClr(DllCall("GetSysColor", "UInt", 15))) ; COLOR_3DFACE
        GuiControl +Background%g_WinColor%, ColorPreview
    ...
    Gui Tab, 5 ; Script
        ...
        Gui Add, CheckBox, vChkSingleInstance x20 y126 w113 h23 Checked, #Single&Instance
        ...
        Gui Add, CheckBox, vChkListLinesOff x144 y207 w113 h23, &ListLines Off
    ...
    Gui Add, Button, gReset x6 y351 w86 h24, &Reset
    Gui Add, Button, gPropertiesClose x97 y351 w86 h24, &Close
    Gui Add, Button, gApplyProperties x188 y351 w86 h24 Default, &Apply

    WinGetPos ax, ay, aw,, ahk_id %hAutoWnd%
    IniRead px, %IniFile%, Properties, x, % (ax + aw - 281 - 40)
    IniRead py, %IniFile%, Properties, y, % ay + 40
    Gui Properties: Show, % "x" . px . " y" . py . " w281 h383", Properties    
    } Else {
...
ApplyProperties:
    ...
    ; Script tab
    Script.NoEnv := ChkNoEnv
    Script.Warn := (ChkWarn) ? CbxWarn : ""
    Script.SingleInstance := (ChkSingleInstance) ? CbxSingleInstance : ""
    Script.NoTrayIcon := ChkNoTrayIcon
    Script.WorkingDir := (ChkSetWorkingDir) ? EdtSetWorkingDir : ""
    Script.SendMode := (ChkSendMode) ? CbxSendMode : ""
    Script.NoBatchLines := ChkSetBatchLines
    Script.ListLinesOff := ChkListLinesOff
    Script.Persistent := ChkPersistent
    Script.IgnoreMenuErrors := ChkIgnoreMenuErrors
    ...
    If (!ChkBGColor) {
        g.Window.Color := ""
        g_WinColor := ToHex(CvtClr(DllCall("GetSysColor", "UInt", 15))) ; COLOR_3DFACE
        GuiControl Properties: +Background%g_WinColor%, ColorPreview
    }
...
Reset(TabIndex) {
    If (TabIndex = 1) {
        ...
        GuiControl,, ChkCenter, 0 ; change to 1 to revert back to original
...
ChooseBGColor:
    ...
    If (ChooseColor(g_WinColor, hPropWnd)) {
        GuiControl +Background%g_WinColor%, ColorPreview
        GuiControl,, ChkBGColor, 1

        If (g_WinColor = 0XFFFFFF) {
            g_WinColor := "White"
        }

        g.Window.Color := "" . g_WinColor
    }
Return
...
SetOption:
    ...
    Option := ControlOptions[MenuItem]
...
SetHintText:
    ...
    If (!ErrorLevel) {
        ...
        If (g[g_Control].hWndVar == "") {
            g[g_Control].hWndVar := "h" . g[g_Control].ClassNN
            GuiControl, Properties:, EdtHWndVar, % g[g_Control].hWndVar
        }
