...
SetWindowPos(hWnd, x, y, w, h, hWndInsertAfter := 0, uFlags := 0x40) { ; SWP_SHOWWINDOW
    Return DllCall("SetWindowPos", "Ptr", hWnd, "UInt", hWndInsertAfter, "UInt", x, "UInt", y, "UInt", w, "UInt", h, "UInt", uFlags)
}
...
MoveWindow(hWnd, x, y, w, h, bRepaint) {
    DllCall("MoveWindow", "Ptr", hWnd, "UInt", x, "UInt", y, "UInt", w, "UInt", h, "UInt", bRepaint)
}
...
LV_GetColumnWidth(hWnd, Column) {
    SendMessage 0x101D, Column - 1, 0,, ahk_id %hWnd% ; LVM_GETCOLUMNWIDTH
    Return (ErrorLevel != "FAIL" ? ErrorLevel : -1)
}

LV_CalcViewSize(hWnd, Items := 0) {
    Result := False
    SendMessage 0x1040, Items - 1, 0, , ahk_id %hWnd% ; LVM_APPROXIMATEVIEWRECT
    If (ErrorLevel != "FAIL") {
        Result := {}
