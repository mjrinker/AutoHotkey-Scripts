...
ControlColor(Control, Window, bc := "", tc := "", Redraw := 1) {
    ...
    WindowProc("Set", a, "", "")
    If (Redraw) {
        NumPut(VarSetCapacity(WINDOWINFO, 60, 0), WINDOWINFO, "UInt")
...
WindowProc(hWnd, uMsg, wParam, lParam) {
    Static Win := {}, SetWindowLong := A_PtrSize == 8 ? "SetWindowLongPtr" : "SetWindowLong"
    Critical
    ...
    If (hWnd == "Set") {
        ...
        If Not Win[a.g, "WindowProcOld"]
            Win[a.g,"WindowProcOld"] := DllCall(SetWindowLong, "Ptr", a.g, "Int", -4, "Int", RegisterCallback("WindowProc", "", 4), "Ptr")
