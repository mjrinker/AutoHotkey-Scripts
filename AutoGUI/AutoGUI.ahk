; AutoGUI v1.4.9a
; Tested on AHK v1.1.25.02 Unicode 32/64-bit, Windows XP/7/10
...
If (A_PtrSize == 8) {
    AddMenu("AutoRunMenu", "&AutoHotkey 64-&bit`tF9", "RunScript", IconLib, 93)
    AddMenu("AutoRunMenu", "AutoHotkey 32-bit`tShift+F9", "RunScript", IconLib, 92)
} Else {
...
Gui Add, Tab2, hWndhTab vTab gTabHandler x164 y32 w650 h448 AltSubmit Theme -Wrap, Untitled 1
...
TabEx := New GuiTabEx(hTab)
    ...
    Sci[1] := New Scintilla
    Sci[1].Add(hAutoWnd, 169, 60, 1576, 1420, SciLexer, 0x50010000, 0)
    SetScintilla(1)
...
If (FileExist(IniFile)) {
    SetWindowPlacement(hAutoWnd, ax, ay, aw, ah, Show)
} Else {
    Gui Show, w841 h561
}
...
If (Files.Length()) {
    ...
    GoSub LoadLastSession
}

SplitPath A_AhkPath,, AhkDir
g_AhkPath3264 := AhkDir . (A_PtrSize == 4 ? "\AutoHotkeyU64.exe" : "\AutoHotkeyU32.exe")

Global g_ThemeFix := DllCall("UxTheme.dll\IsThemeActive") ? "" : "-Theme" ; For Tab3
...
Hotkey IfWinActive, ahk_id %hAutoWnd%
Hotkey #Enter, AutoCompleteWin32
...
CreateToolbox() {
    Gui Add, ListView
    , hWndhToolbox gToolboxHandler x0 y32 w160 h510 AltSubmit -Multi +LV0x10000 Background0xFEFEFE, Control Types
...
TabHandler:
    ...
    If (WrapMode) {
        ...
        Menu AutoOptionsMenu, Uncheck, &Wrap Long Lines
    }
...
CreateToolbar:
    ...
    hToolbar := ToolbarCreate("OnToolbar", TbarButtons, TbarIL, "FLAT LIST TOOLTIPS")
    SendMessage 0x41F, 0, 0x00180018,, ahk_id %hToolbar% ; TB_SETBUTTONSIZE
    WinSet ExStyle, +0x200, ahk_id %hToolbar%
Return
...
CreateEditorToolbar:
    ...
    hEditorTB := ToolbarCreate("OnEditorToolbar", EditorTBBtns, EditorTBIL, "FLAT LIST TOOLTIPS")
    SendMessage 0x41F, 0, 0x00180018,, ahk_id %hEditorTB% ; TB_SETBUTTONSIZE
    WinSet ExStyle, +0x200, ahk_id %hEditorTB%
    ShowWindow(hEditorTB, 0)
Return

OnEditorToolbar(hWnd, Event, Text, Pos, Id) {
    If (Event != "Click") {
        Return
    }
    ...
    If (Text = "New GUI") {
        ...
        GoSub ToggleSyntaxHighlighting
...
NewGUI:
    If (g_GuiTab) {
        MsgBox 0x2031, AutoGUI, Only one GUI can be created at a time. Proceed?
        IfMsgBox Cancel, {
            Return
        }
...
OnWM_MBUTTONDOWN(wParam, lParam, msg, hWnd) {
    ...
    If (hGui != hChildWnd) {
        If (hWnd == hTab) {
            CloseTab(TabHitTest(hTab, lParam & 0xFFFF, lParam >> 16))
        }
        Return
    }
    ...
    ; Main window
        ...
        If (CtrlP) {
            If (wParam == 13) { ; Ctrl+Enter
                ...
                GoSub DuplicateLine
                Return False
            } Else If (wParam == 0x26) { ; Ctrl + Up arrow key
                GoSub TransposeLine
                Return False
            }
        }
...
BlinkBorder(hWnd, Duration := 500, Color := "0x3FBBE3", Radius := 3) {
...
ImportGUI:
    ;GoSub ShowImportGUIDlg ;T his is in the original
    FileSelectFile FileName, 1, %OpenDir%, Open, AutoHotkey Scripts (*.ahk) ; RazorHalo edit
        If (!ErrorLevel) { ; RazorHalo edit
            Open([FileName], 1) ; RazorHalo edit
        } ; RazorHalo edit
Return
...
Open(Files := "", Flag := 0) {
    ...
    For Each, File in Files {
        ...
        If (Flag == 1 && FileExt = "ahk") { ; RazorHalo edit
            n := TabEx.GetSel() ; RazorHalo edit
            If (n == g_GuiTab || Sci[n].GetModify() || Sci[n].FileName != "") { ; RazorHalo edit
                n := NewTab() ; RazorHalo edit
            } ; RazorHalo edit
            
            TabEx.SetText(n, FileName) ; RazorHalo edit
            Gosub NewGUI
            ParseScript(fRead) ; Import GUI
            Sci[n].FullFileName := File ; RazorHalo edit
            Sci[n].FileName := FileName ; RazorHalo edit
            Sci[n].SetText("", fRead, 2) ; RazorHalo edit
            Sci[n].SetSavePoint() ; RazorHalo edit
            Sci[n].EmptyUndoBuffer() ; RazorHalo edit
            Sci[n].Encoding := fEncoding ; RazorHalo edit
        } Else {
            n := TabEx.GetSel()
...
Save() {
    ...
    If (g_Backup) {
        If (!FileExist(g_BkpDir)) {
            FileCreateDir %g_BkpDir%
        }
        FileAppend %SciText%, % GetTempFileName(g_BkpDir)
    }
    ...
    FileGetTime Timestamp, %FullFileName%
    Sci[n].Timestamp := Timestamp
...
RunScript(Mode := 1) {
    ...
    If (Mode == 7) {
        ExecScript("MsgBox 0, AutoGUI, % " . GetSelectedText(), g_Parameters, AhkPath)
        Return
    }

    If (Mode == 6) { ; Run to Cursor
        Sci[n].GetTextRange([0, Sci[n].GetCurrentPos()], TextToCursor)
        ExecScript(TextToCursor, g_Parameters, AhkPath)
        Return
    }

    Sci[n].GetText(Size + 1, SciText)

    If (Mode == 5) { ; Run Selected Text (F5)
...
ShowParamsDlg:
    ...
    Temp := InputBoxEx("Script Parameters", Info, "Command Line Parameters", g_Parameters, "", "", 440, "", "", hAutoWnd, IconLib, 91)
...
Compile:
    ...
    n := TabEx.GetSel()
    If (Sci[n].FullFileName && !Sci[n].GetModify()) {
        SplitPath % Sci[n].FullFileName,, Dir,, NameNoExt
        ...
        If (!ErrorLevel) {
            ControlSetText Edit1, % Sci[n].FullFileName, ahk_id %hWnd%
        }
...
HelpMenuHandler:
    ...
    If (SubStr(URL, 1, 1) == "/") {
        Run HH mk:@MSITStore:%HelpFile%::%URL%
    } Else {
        Try {
            Run %URL%
        } Catch e {
...
OnWM_LBUTTONUP(wParam, lParam, msg, hWnd) {
    ...
    If (hWnd == hTab && g_PrevCtrl == hTab) {
        mx := lParam & 0xFFFF
        my := lParam >> 16
        HotTab := TabHitTest(hTab, mx, my)
        CurTab := TabEx.GetSel()
        If (CurTab != HotTab) {
            HotTabTitle := TabEx.GetText(HotTab)
            TabEx.SetText(HotTab, TabEx.GetText(CurTab))
            TabEx.SetText(CurTab, HotTabTitle)

            ShowWindow(Sci[HotTab].hWnd)
            ShowWindow(Sci[CurTab].hWnd, 0)

            Temp := Sci[HotTab]
            Sci[HotTab] := Sci[CurTab]
            Sci[CurTab] := Temp
...
ReplaceConstant:
    ...
    If (Constant ~= "\d+" || Value == "") {
        Run % A_ScriptDir . "\Tools\Constantine.ahk " . RegExReplace(Constant, "0X", "0x"), Tools
        Return
    }
...
CheckTimestamp() {
    ...
    If (Sci[n].FileName != "" && !FileExist(Sci[n].FullFileName)) {
        OnMessage(0x1C, "")
        ;Gui Auto: +OwnDialogs
        MsgBox 0x40030, %AppName%, % "File not found: " . Sci[n].FullFileName
        Sci[n].Timestamp := ""
        OnMessage(0x1C, "OnWM_ACTIVATEAPP")
        Return
    }
    ...
    If (g_CheckTimestamp) {
        Filename := Sci[n].FullFileName

        FileGetTime Timestamp, %Filename%
        If (Timestamp != Sci[n].Timestamp) {
            OnMessage(0x1C, "") ; Prevent this function from being called twice when the MsgBox is closed.

            File := Sci[n].Filename
            ;Gui Auto: +OwnDialogs
            MsgBox 0x40034, %AppName%, %File% was modified by another application.`nShould the file be reloaded?
            IfMsgBox Yes, {
...
RunTool:
    ...
    If (Params != "") {
        ...
        If (InStr(Params, "{FILENAME}")) {
            Params := StrReplace(Params, "{FILENAME}", Sci[n].FullFilename)
        }

        If (InStr(Params, "{FILEDIR}")) {
            SplitPath % Sci[n].FullFilename,, FileDir
...
ShowIncludesDialog:
    ...
    Gui Add, Text, x12 y8 w120 h22 +0x200, List of Includes
    Gui Font
    Gui Font, s9, Segoe UI
    Gui Add, Text, x12 y32 w285 h20 +0x200, Select the files to be opened
    Gui Add, ListView, hWndhIncludeList x10 y59 w560 h280 +LV0x14000 +Checked, Filename|Path
    Gui Add, Text, x-1 y350 w582 h48 +Border -Background
    Gui Add, Button, gOpenIncludes x396 y362 w84 h24 +Default, &OK
    Gui Add, Button, gIncludesDlgClose x486 y362 w84 h24, &Cancel
    Gui Font
    Gui Show, w580 h397, Open Included File

    LV_ModifyCol(1, 160)
    LV_ModifyCol(2, 375)
    DllCall("UxTheme.dll\SetWindowTheme", "Ptr", hIncludeList, "WStr", "Explorer", "Ptr", 0)

    FullFilename := Sci[TabEx.GetSel()].Fullfilename
    If (FullFilename != "") {
        Try {
            EnumIncludes(FullFilename, Func("EnumIncludesCallback"))
        }
    }
Return
...
LoadSessionMenu:
    ...
    Loop %g_SessionsDir%\*.session
    {
        ...
        Menu AutoSessionMenu, Add, %NameNoExt%, LoadSession
        ItemCount++
    }

    If (ItemCount > 1) {
        Menu AutoSessionMenu, Delete, %MostRecentFile%
        Menu AutoSessionMenu, Insert, 1&, %MostRecentFile%, LoadSession
    }
...
LoadLastSession:
    ...
    SessionFiles := []
    FileRead Session, %MostRecentFile%
    Loop Parse, Session, `n, `r
    {
        If (FileExist(A_LoopField)) {
            SessionFiles.Push(A_LoopField)
        }
    }
...
SaveSession:
    Session := ""
    Loop % Sci.Length() {
        Filename := Sci[A_Index].FullFilename
        If (FileExist(Filename)) {
            Session .= Filename . CRLF
        }
    }
    ...
    If (Session != "") {
        ...
        FileSelectFile SelectedFile, S16, %Filename%, Save Session, Session Files (*.session)
        If (!ErrorLevel) {
            SplitPath SelectedFile,,, Ext
            If (!(Ext = "session") && !FileExist(SelectedFile . ".session")) {
                SelectedFile .= ".session"
            }

            FileDelete %SelectedFile%
            FileAppend %Session%, %SelectedFile%
            If (ErrorLevel) {
                Gui Auto: +OwnDialogs
