ShowToolbarEditor:
    If (!WinExist("ahk_id " . hToolbarEditor)) {
        ...
        Gui Add, ListView
            , hWndhToolbarButtonsList gToolbarButtonsListHandler x10 y10 w214 h264 +NoSortHdr -Multi +LV0x10000 +0x200
            , Toolbar Items
        LV_ModifyCol(1, 192)
        ...
        Gui Add, GroupBox, x236 y110 w188 h164, Toolbar Options
            Gui Add, CheckBox, vTB_ChkAdjustable x250 y132 w80 h23, Adjustable
            Gui Add, CheckBox, vTB_ChkBottom x250 y186 w80 h23, Bottom
            Gui Add, CheckBox, vTB_ChkBorder x250 y159 w80 h23, Border
            Gui Add, CheckBox, vTB_ChkFlat x250 y213 w80 h23 +Checked, Flat
            Gui Add, CheckBox, vTB_ChkList x250 y240 w80 h23 +Checked, List
            Gui Add, CheckBox, vTB_ChkMenu x339 y132 w80 h23, Text Only
            Gui Add, CheckBox, vTB_ChkNoDivider x338 y159 w80 h23, No Divider
            Gui Add, CheckBox, vTB_ChkTabstop x338 y186 w80 h23, Tabstop
            Gui Add, CheckBox, vTB_ChkTooltips x338 y213 w80 h23 +Checked, Tooltips
            Gui Add, CheckBox, vTB_ChkVertical x338 y240 w80 h23, Vertical
        ...
        If (g_NT6orLater) {
            DllCall("UxTheme.dll\SetWindowTheme", "Ptr", hToolbarButtonsList, "WStr", "Explorer", "Ptr", 0)
        }
    } Else {
...
EditToolbarButton:
    ...
    If (A_ThisLabel == "EditToolbarButton") {
        ...
        ;ToolbarButtonIcon := Toolbar.Buttons[Index].Icon ; . "||"
    ...
    ChkStayPressed  := (RegExMatch(Toolbar.Buttons[Index].Style, "i)\bCHECK\b")) ? 1 : 0
    ChkGrouped      := (InStr(Toolbar.Buttons[Index].Style, "CHECKGROUP")) ? 1 : 0
    ChkDropDown     := (InStr(Toolbar.Buttons[Index].Style, "DROPDOWN")) ? 1 : 0
    ChkShowText     := (InStr(Toolbar.Buttons[Index].Style, "SHOWTEXT")) ? 1 : 0
    ChkStartPressed := (InStr(Toolbar.Buttons[Index].State, "CHECKED")) ? 1 : 0
    ChkDisabled     := (InStr(Toolbar.Buttons[Index].State, "DISABLED")) ? 1 : 0
    ChkHidden       := (InStr(Toolbar.Buttons[Index].State, "HIDDEN")) ? 1 : 0
    ChkbreakRow     := (InStr(Toolbar.Buttons[Index].State, "WRAP")) ? 1 : 0
...
NewToolbarButtonDlgEdit:
    ...
    If (A_ThisLabel == "NewToolbarButtonDlgNext") {
        ...
        Gui ToolbarEditor: Default
        GuiControl Focus, %hToolbarButtonsList%
...
ToolbarEditorOK:
    ...
    If (Toolbar.Buttons.Length()) {
        AddToolbar(ImageList2, ToolbarButtons, Toolbar.Options)
...
ToolbarButtonsListHandler:
    If (A_GuiEvent == "e") {
        ...
        PostMessage 0x1076, Row, 0,, ahk_id %hToolbarButtonsList% ; LVM_EDITLABELW
    }
Return
...
ToolbarEditorHelp:
    Try {
        Run %A_ScriptDir%\Help\Toolbar Editor Help.ahk
    }
Return
