; AutoHotkey Scripts Manager 1.0
...
Global AppName := "AutoHotkey Scripts Manager"
    ...
    , IconLib := "..\Icons\ScriptsManager.icl"
...
Gui Add, ListView, hWndhLV gLVHandler x0 y28 w620 h285 +LV0x14000 AltSubmit, Filename|Path|PID
LV_ModifyCol(1, 174)
LV_ModifyCol(2, 387)
...
Gui Show, w620 h336, AutoHotkey Scripts Manager
...
Global hToolbar := GuiAddToolBar(hMainWnd)
...
GuiAddToolbar(hWnd) {
    Local ImageList := IL_Create(7)
    IL_Add(ImageList, IconLib, 2)
    IL_Add(ImageList, IconLib, 3)
    IL_Add(ImageList, IconLib, 4)
    IL_Add(ImageList, IconLib, 5)
    IL_Add(ImageList, IconLib, 6)

    IL_Add(ImageList, IconLib, 7)
    IL_Add(ImageList, IconLib, 8)
    IL_Add(ImageList, IconLib, 9)
    IL_Add(ImageList, IconLib, 10)

    IL_Add(ImageList, IconLib, 11)
    IL_Add(ImageList, IconLib, 12)
...
UpdateToolbar:
    ...
    SendMessage 0x402, 3, % GetScriptState(PID, "S"),, ahk_id %hToolbar% ; TB_CHECKBUTTON
    SendMessage 0x402, 4, % GetScriptState(PID, "P"),, ahk_id %hToolbar% ; Pause is set on WM_ENTERMENULOOP
Return

ShowAbout:
    Gui +OwnDialogs
    MsgBox 0x40, About, AutoHotkey Scripts Manager 1.0
Return
...
EnsureU32() {
    If (A_PtrSize == 8 || !A_IsUnicode) {
        If (FileExist(U32 := A_AhkPath . "\..\AutoHotkeyU32.exe")) {
            ...
            MsgBox 0x10, AutoHotkey Scripts Manager, AutoHotkey 32-bit Unicode not found.
        }
        ExitApp
    }
}
...
GetCPUUsage(PID) {
    ...
    Return CPUUsage := (CPUUsage) ? CPUUsage : "0.01"
}
...
ExecCommand(Command) {
    ...
    If ((Command == "Reload Script" || Command == "Exit Script") && !GetKeyState("Shift", "P")) {
        ...
        MsgBox 0x31, AutoHotkey Scripts Manager, % "Are you sure you want to " . Verb . " " . Filename . "?"
