...
Loop %0% {
    SFN := %A_Index%
    Loop %SFN%, 1
        LFN = %A_LoopFileLongPath%
    Files.Push(LFN)
}
0 := 0

If (Files.Length() && hPrevInstance := WinExist("AutoGUI v")) {
    Loop 10 {
...
, g_Delimiter := "; End of the GUI section"   ;RH
, NoGrippers
, Grippers := []
, hGripper1
, hGripper2
...
Gui Add, Tab2, hWndhTab vTab gTabHandler x164 y32 w650 h448 AltSubmit Theme -Wrap +0x4000000, Untitled 1
...
LoadRecentFiles() {
    ...
    If (Recent != "ERROR") {
        Loop Parse, Recent, `n
        {
            RecentFile := StrSplit(A_LoopField, "=")[2]
...
ReplaceConstant:
    ...
    If (Constant ~= "\d+" || Value == "") {
        Run % A_ScriptDir . "\Tools\Constantine.ahk " . RegExReplace(Constant, "X", "x"), Tools
        Return
    }
...
CheckTimestamp() {
    n := TabEx.GetSel()
    If (g_CheckTimestamp && Sci[n].Modified != "") {
        Filename := Sci[n].FullFileName

        FileGetTime Timestamp, %Filename%
        If (Timestamp != Sci[n].Modified) {
            ; Prevent this function from being called twice when the MsgBox is closed.
            g_SkipTimestamp := True

            File := Sci[n].Filename
            MsgBox 0x2034, %AppName%, %File% was modified by another application.`nShould the file be reloaded?
            IfMsgBox Yes, {
